...
function renderEpfPage() {
  ...
  summaryFrame.innerHTML = `
    <div class="frame-head"><span class="title">accounts — ${holdings.length}</span></div>
    <div class="frame-body table-scroll">
      <table>
        ...
        <tbody>
          ${rows.length ? rows.map(({ h, val }) => `
            <tr>
              <td><a href="#acc-${h.id}">${h.employerName}</a>${h.uan ? `<br><span style="color:var(--text-faint);font-size:11px;">UAN ${h.uan}</span>` : ''}</td>
...
function buildAccountCard(h) {
  ...
  wrap.innerHTML = `
    <div class="frame-head">
      <span class="title">${h.employerName}</span>
      <button data-del="${h.id}" class="danger" style="padding:3px 8px;font-size:11px;">delete account</button>
    </div>
    <div class="frame-body">
      ...
      <div style="display:grid; grid-template-columns: 1fr 1fr; gap:16px;" class="epf-grid">
        <div>
          <h4 style="margin:0 0 8px; font-size:12px; color:var(--text-dim); text-transform:uppercase;">monthly contribution</h4>
          <div class="inline-form" style="margin-bottom:6px;">
            <label style="display:flex; align-items:center; gap:5px; font-size:12px;"><input type="radio" name="mode-${h.id}" value="manual" ${rec.mode !== 'salary' ? 'checked' : ''} style="width:auto;" /> manual amounts</label>
            <label style="display:flex; align-items:center; gap:5px; font-size:12px;"><input type="radio" name="mode-${h.id}" value="salary" ${rec.mode === 'salary' ? 'checked' : ''} style="width:auto;" /> calculate from salary</label>
          </div>
          ...
          <div id="salaryBlock-${h.id}" style="${rec.mode === 'salary' ? '' : 'display:none;'}">
            ...
            <div id="salBreakdown-${h.id}" style="font-size:11px; color:var(--text-dim); margin-bottom:8px; line-height:1.6;"></div>
            <p style="color:var(--text-faint); font-size:10px; margin:0 0 8px;">Standard EPFO split: employee 12% of basic; employer 12% of basic split into 8.33% (capped ₹1,250) to EPS and the remainder to EPF. Uses basic+DA, not gross salary.</p>
          </div>

          <div class="form-field" style="margin:8px 0;"><label>effective from</label><input id="recStart-${h.id}" type="date" value="${rec.startDate || Finance.todayStr()}" /></div>
          ...
          <button id="saveRec-${h.id}">save contribution setup</button>
        </div>

        <div>
          <h4 style="margin:0 0 8px; font-size:12px; color:var(--text-dim); text-transform:uppercase;">declared interest rate by FY</h4>
          <div id="ratesTable-${h.id}" style="margin-bottom:10px;"></div>
      ...
      <div style="margin-top:20px;">
        <h4 style="margin:0 0 8px; font-size:12px; color:var(--text-dim); text-transform:uppercase;">balance projection</h4>
        <div class="form-grid" style="margin-bottom:10px;">
          ...
          <div class="form-field"><button id="projBtn-${h.id}">update projection</button></div>
        </div>
        <div class="chart-box" id="projChart-${h.id}"></div>
        <div id="projFinal-${h.id}" style="margin-top:8px; font-size:13px;"></div>
      </div>
    </div>
  `;
    ...
    rt.innerHTML = `
      <table>
        ...
        <tbody>${list.length ? list.map((r, i) => `
          <tr><td>${r.fyLabel}</td><td class="num">${r.ratePct}%</td><td><button data-ridx="${holding.interestRates.indexOf(r)}" class="danger" style="padding:2px 6px;font-size:11px;">del</button></td></tr>
  ...
  function runProjection() {
    ...
    wrap.querySelector(`#projFinal-${h.id}`).innerHTML =
      `projected balance in ${yearsForward} years: <b style="font-size:16px;">${Fmt.money(proj.finalBalance)}</b>
...
// avoid full re-render on every keystroke-triggered save (would wipe in-progress
// form state like radio selection); explicit actions already call renderEpfPage()
// themselves, this just keeps the topbar's dirty/source indicator accurate
