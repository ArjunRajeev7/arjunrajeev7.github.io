...
function renderFdPage() {
  ...
  // maturity timeline — sorted list with countdown
  ...
  if (!rows.length) {
    ...
    const maxDays = Math.max(...sorted.map(r => Finance.daysBetween(r.h.startDate, r.val.maturityDate)), 1);
      ...
      return `
        <div style="margin-bottom:14px;">
          <div style="display:flex; justify-content:space-between; font-size:12px; margin-bottom:4px;">
            ...
            <span>${Fmt.date(h.startDate)} → ${Fmt.date(val.maturityDate)}</span>
          </div>
          <div style="background:#1a1a1a; border:1px solid #2b2b2b; height:10px; position:relative;">
            <div style="background:#fff; width:${pct}%; height:100%;"></div>
          </div>
        </div>
      `;
    }).join('');
...
(async () => {
  await Store.init();
  renderTopbar('fixed-deposits.html');
