...
function renderMfPage() {
  ...
  Charts.renderDonut(allocHolder, rows.map(r => ({ label: r.h.name, value: r.val.currentValueINR || 0 })), {
    r: 60, cx: 76, cy: 76, strokeW: 22, centerLine1: holdings.length + ' funds', ariaLabel: 'allocation across mutual funds'
  });
      ...
      <table>
        ...
        <tbody>
          ${rows.length ? rows.map(({ h, val }) => `
            <tr>
              <td>${h.name}<br><span style="color:var(--text-faint);font-size:11px;">code ${h.schemeCode}${h.folio ? ' · folio ' + h.folio : ''}</span>
              ...
              <td class="num">${val.priceInfo ? Fmt.moneyPrecise(val.priceInfo.price) : '—'}</td>
...
function openManualPriceModal(assetType, h) {
  openModal(`manual price — ${h.symbol || h.name}`, `
    <p style="color:var(--text-dim); font-size:12px; margin-top:0;">Use this if the live API can't find this scheme. It's used until the next successful live fetch overrides it.</p>
...
(async () => {
  await Store.init();
  renderTopbar('mutual-funds.html');
