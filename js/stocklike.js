...
function renderStockLikePage(assetType) {
  ...
  if (allocHolder) {
    Charts.renderDonut(allocHolder, rows.map(r => ({ label: r.h.symbol, value: r.val.currentValueINR || 0 })), {
      r: 60, cx: 76, cy: 76, strokeW: 22, centerLine1: holdings.length + ' held', ariaLabel: 'allocation within ' + ASSET_LABELS[assetType]
    });
  ...
  addFrame.innerHTML = `
    <div class="frame-head"><span class="title">add ${isUS ? 'us stock / etf' : 'nse stock'}</span></div>
    <div class="frame-body">
      <div class="inline-form">
        ...
        <div class="form-field" style="flex:0;"><button id="addHoldingBtn">+ add</button></div>
      </div>
    </div>
  `;
  ...
  tableFrame.innerHTML = `
    <div class="frame-head"><span class="title">holdings — ${holdings.length}</span></div>
      ...
      <table>
        ...
        <tbody>
          ${rows.length ? rows.map(({ h, val }) => {
            ...
            return `
            <tr>
              <td>${h.symbol}${h.name && h.name !== h.symbol ? `<br><span style="color:var(--text-faint);font-size:11px;">${h.name}</span>` : ''}
                ${!val.hasPrice ? '<span class="badge">no price</span>' : ''}</td>
              ...
              <td class="num">${val.priceInfo ? Fmt.moneyPrecise(val.priceInfo.price, currency) : '—'}</td>
...
function openManualPriceModal(assetType, h) {
  openModal(`manual price — ${h.symbol}`, `
    <p style="color:var(--text-dim); font-size:12px; margin-top:0;">Use this if the live API can't find this symbol. It's used until the next successful live fetch overrides it.</p>
    <div class="form-field" style="margin-bottom:12px;">
      <label>price per unit</label>
      <input id="manPrice" type="number" step="0.01" value="${h.manualPrice || ''}" />
    </div>
    <button id="saveManPrice">save</button>
  `, (overlay) => {
  ...
  const render = () => {
    ...
    const txnRows = holding.txns.map(t => `
      <tr>
        ...
        <td><button data-tid="${t.id}" class="danger" style="padding:2px 6px;font-size:11px;">del</button></td>
      </tr>
    ...
    return `
      ...
      <div class="form-grid">
        ...
        <div class="form-field"><button id="addTxnBtn">+ add txn</button></div>
      </div>
    `;
  };
